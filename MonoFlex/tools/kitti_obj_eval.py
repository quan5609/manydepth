...
def train(cfg, model, teacher_model, device, distributed):
...
def eval(cfg, model, teacher_model, device, distributed):
    checkpointer = DetectronCheckpointer(
    ...
    return run_test(cfg, checkpointer.model, teacher_model, vis=False, eval_score_iou=True, eval_all_depths=False)
