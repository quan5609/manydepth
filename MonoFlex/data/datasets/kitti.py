...
class KITTIDataset(Dataset):
	def __init__(self, cfg, root, is_train=True, transforms=None, augment=True):
		...
		self.root = root
		# !HARDCODE Change object image dir to kitti raw dir
		self.image_dir = os.path.join(root, "image")
		self.kitti_raw_dir = os.path.join('/workspace/datasets/kitti_data')
		mapping_file = readlines('splits/kitti_obj/mapping/train_rand.txt')
		self.mapping_idx = [int(x) for x in mapping_file[0].split(',')]

		kitti_raw_mapping_file = readlines('splits/kitti_obj/mapping/train_mapping.txt')
		self.kitti_raw_mapping = [x.strip() for x in kitti_raw_mapping_file]
	...
	def get_kitti_raw_path(self, idx):
		obj_img = self.image_files[idx]
		obj_img_id = int(obj_img.split('.')[0])
		kitti_img = self.kitti_raw_mapping[self.mapping_idx[int(obj_img_id)]-1]
		# print("full", kitti_img)
		return kitti_img

	def get_image(self, idx):
		kitti_date, kitti_drive, kitti_id = self.get_kitti_raw_path(idx).split(" ")
		# print(kitti_folder, kitti_id)
		img_filename = os.path.join(self.kitti_raw_dir, kitti_date, kitti_drive, 'image_02/data', kitti_id + '.png')
		img = Image.open(img_filename).convert('RGB')

		prev_img_filename = os.path.join(self.kitti_raw_dir, kitti_date, kitti_drive, 'image_02/data', str(int(kitti_id)-1).zfill(10) + '.png')
		prev_img = Image.open(prev_img_filename).convert('RGB')
		return img, prev_img
	...
	def __getitem__(self, idx):
		...
		if idx >= self.num_samples:
			...
			img, prev_img = self.get_image(idx)
		...
		if self.augmentation is not None:
			# prev_objs, prev_calib = objs.copy(), calib.copy()
			img, prev_img, objs, calib = self.augmentation(img, prev_img, objs, calib)
			# prev_img, _, _ = self.augmentation(prev_img, prev_objs, prev_calib)
		...
		# pad image
		prev_img_before_aug_pad = np.array(prev_img).copy()
		prev_img_w, prev_img_h = prev_img.size
		prev_img, pad_size = self.pad_image(prev_img)
		# for training visualize, use the padded images
		ori_prev_img = np.array(prev_img).copy() if self.is_train else prev_img_before_aug_pad
		...
		if self.split == 'test':
			...
			target.add_field("ori_prev_img", ori_prev_img)
			if self.enable_edge_fusion:
			...
			if self.transforms is not None: 
				...
				img, prev_img, target = self.transforms(img, prev_img, target)
				# prev_img, _ = self.transforms(prev_img, prev_target)

			return img, prev_img, target, original_idx
