...
class Compose(object):
    ...
    def __call__(self, img, prev_img, objs, calib):
        if isinstance(img, np.ndarray):
            img = Image.fromarray(img, mode="RGB")
            prev_img = Image.fromarray(prev_img, mode="RGB")
            self.PIL2Numpy = True

        for a in self.augmentations:
            img, prev_img, objs, calib = a(img, prev_img, objs, calib)

        if self.PIL2Numpy:
            img = np.array(img)
            prev_img = np.array(prev_img)

        return img, prev_img, objs, calib
